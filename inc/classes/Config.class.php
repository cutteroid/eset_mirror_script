...
class Config {
	...
	static private $DEFAULT_CONF = array(
		...
		'icq_informer_enable' => '0',
		'icq_informer_codepage' => 'UTF-8',
		'icq_informer_level' => '2',
		'icq_informer_days' => '3',
		'icq_informer_login' => null,
		'icq_informer_password' => null,
		'icq_informer_destination' => null,
		'phpmailer_enable' => '0',
	...
	static public function check_config() {
		...
		if(self::$CONF['log_rotate_enable'] == 1) {
			if(preg_match_all("/([0-9]+)([BKMG])/i",self::$CONF['log_rotate_size'],$result,PREG_PATTERN_ORDER)) {
				...
				switch(trim($result[2][0])) {
					case "G": self::$CONF['log_rotate_size'] = self::$CONF['log_rotate_size'] * 1024;
					case "M": self::$CONF['log_rotate_size'] = self::$CONF['log_rotate_size'] * 1024;
					case "K": self::$CONF['log_rotate_size'] = self::$CONF['log_rotate_size'] * 1024;
				}
			}
		...
		if(self::$CONF['icq_informer_enable'] == 1) {
			self::$CONF['icq_informer_password'] = strval(self::$CONF['icq_informer_password']);
			if(empty(self::$CONF['icq_informer_login']) || empty(self::$CONF['icq_informer_password']) || empty(self::$CONF['icq_informer_destination'])) {
				return "You enabled 'icq_informer_enable',but didn't set up 'icq_informer_login' or 'icq_informer_password' or 'icq_informer_destination'.Please, check your config file!";
			}
		}
		...
		if ($test == false) {
			...
			if (function_exists('link') && link(Tools::ds(self::$CONF['web_dir'],'linktest'), Tools::ds(self::$CONF['web_dir'],'linktest2'))) $status = 'link';
			elseif (preg_match("/^win/i", PHP_OS) && shell_exec(sprintf("fsutil hardlink create %s %s", Tools::ds(self::$CONF['web_dir'],'linktest'), Tools::ds(self::$CONF['web_dir'],'linktest2'))) != 0) $status = 'fsutil';
			else $status = 'false';
			if  ($status) unlink(Tools::ds(self::$CONF['web_dir'],'linktest2'));
