...
class Nod32ms {
		...
		while( false !== ($entry = $d->read()) ) {
		  if(($entry == '.') || ($entry == '..')) continue;
				if (is_dir(Tools::ds($directory, $entry))) {
					get_all_patterns(Tools::ds($directory, $entry));
					continue;
				}
    ...
    static private function strip_tags_and_css($search) {
		$document = array ("'<script[^>]*?>.*?</script>'si", "'<[\/\!]*?[^<>]*?>'si", "'([\r\n])[\s]+'", "'&(quot|#34);'i", "'&(amp|#38);'i", "'&(lt|#60);'i", "'&(gt|#62);'i", "'&(nbsp|#160);'i", "'&(iexcl|#161);'i", "'&(cent|#162);'i", "'&(pound|#163);'i", "'&(copy|#169);'i", "'&#(\d+);'e");
	...
	private function generate_html($time_run_script) {
		...
		if(Config::get('generate_only_table') == '0') {
			$html_page .= '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd"><html><head><title>'.Language::t("ESET NOD32 update server").'</title><meta http-equiv="Content-Type" content="text/html; charset='.Config::get('html_codepage').'"><style type="text/css">html,body{height:100%;margin:0;padding:0;width:100%}table#center{border:0;height:100%;width:100%}table td table td{text-align:center;vertical-align:middle;font-weight:bold;padding:10px 15px;border:0}.gray{background:#eee}.orange{background:#fc0}</style></head><body><table id="center"><tr><td align="center">';
		}
		...
		foreach($DIRECTORIES as $ver => $dir) {
			if(Config::upd_version_is_set($ver) == '1') {
				...
				$html_page .= '<tr'.(((Config::get('generate_only_table') == "0") and ($i % 2 != 0)) ? ' class="gray"' : '').'>'; $i++;
		...
		$html_page .= '<td colspan="2"'.(((Config::get('generate_only_table') == "0") and ($i % 2 != 0)) ? ' class="gray"' : '').'>'; $i++;
